...
//////////////////////////bkjung///////////////////////////////////
bool Turtlebot3MotorDriver::writePosition(int wheel_select, int32_t wheel_angle)
{
  uint8_t dxl_error = 0;
  int8_t dxl_comm_result = false;
  if (wheel_select == LEFT)
    dxl_comm_result = packetHandler_->write4ByteTxRx(portHandler_, 1, ADDR_X_GOAL_POSITION, wheel_angle, &dxl_error);
  else if (wheel_select == RIGHT)
    dxl_comm_result = packetHandler_->write4ByteTxRx(portHandler_, 2, ADDR_X_GOAL_POSITION, wheel_angle, &dxl_error);
  else
    return false;
    //ERROR
    ;
...
bool Turtlebot3MotorDriver::controlMotorAngle(int32_t left_wheel_angle, int32_t right_wheel_angle)
{
  bool dxl_comm_result = false;
  
  //limit left_wheel_angle value range in extended position control mode
  if (left_wheel_angle < (int32_t)-1048575)
    left_wheel_angle = (int32_t)-1048575;
  else if (left_wheel_angle > (int32_t)1048575)
    left_wheel_angle = (int32_t)1048575;
  else
    ;

  //limit right_wheel_angle value range in extended position control mode
  if (right_wheel_angle < (int32_t)-1048575)
    right_wheel_angle = (int32_t)-1048575;
  else if (right_wheel_angle > (int32_t)1048575)
    right_wheel_angle = (int32_t)1048575;
  else
    ;


  dxl_comm_result = writePosition(LEFT, left_wheel_angle);
  dxl_comm_result = writePosition(RIGHT, right_wheel_angle);
